...
export class ProductManager {
    ...
    async addProduct(newProduct) {
        ...
        if (newProduct.title && newProduct.description && newProduct.price && newProduct.code && newProduct.stock) {
            ...
            if (indice === -1) {
                newProduct.status = true // Valor definido por la 1ra entrega
                if(!newProduct.thumbnail){ // No pido una imagen, sin embargo, si no hay ninguna pongo una por defecto
